<?php
// api/square-payment.php - Updated with proper email functionality
...
try {
    ...
    try {
        ...
        // Schedule emails like the original working version
        ...
        // Send confirmation email immediately like the original version
        ...
        // Prepare booking array for email template (same format as original)
        $emailBooking = [
        ...
        if ($emailSent) {
            ...
            error_log("Square payment - Failed to send confirmation email");
        }

        // Send housekeeping notice to housekeeping email if defined
        if (defined('HOUSEKEEPING_EMAIL') && !empty(HOUSEKEEPING_EMAIL)) {
            $housekeepingEmailSent = sendHousekeepingNoticeEmail(
                HOUSEKEEPING_EMAIL,
                $resNum,
                $emailBooking
            );
            
            if ($housekeepingEmailSent) {
                error_log("Square payment - Housekeeping notice sent");
            } else {
                error_log("Square payment - Failed to send housekeeping notice");
            }
        }
